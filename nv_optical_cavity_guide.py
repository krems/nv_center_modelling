...
def right_part(t, y):
    hamiltonian = array(
        [[1.5 * w_c - 1j * kappa, -.5 * omega_p * exp(-1j * t * (w_p - w_g)) - 1j * gm * exp(-1j * t * (w_c - w_g)), -.5 * omega_p * exp(-1j * t * (w_m - w_g)),
          0., 0., 0.],
         [-.5 * omega_p * exp(1j * t * (w_p - w_g)) + 1j * gm * exp(1j * t * (w_c - w_g)), 1.5 * w_c - 1j * kappa - w_p, 0.,
          -.5 * omega_m * exp(1j * t * (w_p - w_c)), 0., 0.],
         [-.5 * omega_p * exp(1j * t * (w_m - w_g)), 0., 1.5 * w_c - 1j * kappa + delta - w_m - 1j * gama,
          -.5 * omega_m * exp(1j * t * (w_m - w_c)) - 1j * gm * exp(-1j * t * (w_c - w_g)), 0., 0.],
         [0., -.5 * omega_m * exp(-1j * t * (w_p - w_c)), -.5 * omega_m * exp(-1j * t * (w_m - w_c)) + 1j * gm * exp(1j * t * (w_c - w_g)),
          .5 * w_c, -.5 * omega_p * exp(-1j * t * (w_p - w_g)), -.5 * omega_p * exp(-1j * t * (w_m - w_g))],
         [0., 0., 0.,
          -.5 * omega_p * exp(1j * t * (w_p - w_g)), .5 * w_c - w_p, - 1j * gm * exp(-1j * t * (w_c - w_g))],
         [0., 0., 0.,
          -.5 * omega_p * exp(1j * t * (w_m - w_g)),  1j * gm * exp(1j * t * (w_c - w_g)), .5 * w_c + delta - w_m - 1j * gama]], dtype=complex128)
    hamiltonian *= -1j * h
