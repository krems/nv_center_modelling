...
w_p = 4.8367 * (10 ** 6) + h * mu * B_z  # e->u transition frequency
...
def right_part(t, y):
    # a = 700
    hamiltonian = array(
        [[delta, -h * E / 2., -h * D / 2. - h * mu * B_z, -omega_p / 2. * exp(1j * t * (w_p - w_f))],
         [-h * E / 2., -w_m, -h * D / 2. - h * mu * B_z, -omega_s / 2. * exp(1j * t * (w_m - w_f))],
         [-h * D / 2. - h * mu * B_z, -h * D / 2. - h * mu * B_z, -((w_p + w_m) / 2 - h * D), -omega_z / 2. * exp(1j * t * (w_z - w_f))],
         [-omega_p / 2. * exp(-1j * t * (w_p - w_f)), -omega_s / 2. * exp(-1j * t * (w_m - w_f)), -omega_z / 2. * exp(-1j * t * (w_z - w_f)), delta - 1j * gama]],
        dtype=complex128)
    hamiltonian *= -1j / h
